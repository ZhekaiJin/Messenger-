...
export function user(state=initState, action){
	switch(action.type){
		case AUTH_SUCCESS: //integrate login and register success to one
...
function authSuccess(obj){
	const {pwd,...data} = obj //get red of passwd
...
export function login({user,pwd}){ //control the login component
	...
	return dispatch=>{
		axios.post('/user/login',{user,pwd})
			.then(res=>{
				if (res.status==200&&res.data.code===0) {
					dispatch(authSuccess(res.data.data)) //return data from backend
				}else{
					dispatch(errorMsg(res.data.msg))
				}
			})
